...
############## map 1 to many relationship
## data<-gene.aliases
## un.wind.col<-"Previous.Symbols"
### five table data
unwind<- function(data, un.wind.col,delimit=", ") {
## data[,c(un.wind.col)][1:50]
## is.null(data[,c(un.wind.col)])[1:50]
data[data[,c(un.wind.col)]=="",c(un.wind.col)]<-NA ## for cases where un.wind.col is empty but what to keep that line as someting in other columns NA is of length one
the.genes<-strsplit(data[,c(un.wind.col)],split=delimit)
#unique(unlist(the.genes))
## the.genes[1:50]
## list.element.lengths[1:50]
## data.flat[1:50,]
list.element.lengths<-unlist(lapply(the.genes,length))        # list.lentghs
#list.element.lengths[list.element.lengths==0] <- 1 
 indel.index<-rep(1:length(list.element.lengths),times=list.element.lengths)           # index

data.flat<-data[indel.index,]
the.genes<-unlist(the.genes)
#cbind(the.genes,data.flat)[,1:2]

data.flat[,un.wind.col]<-the.genes
return(data.flat)
}
...
## root<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/SNPs/test"
## root<-"/media/UQCCG/GWAS/Popularion stratification/test3"
## the.indel<-a.indel
 write.plink.ped<-function (the.indels,root) ## note this works with my read.plink function
{
...
the.indel[1:3,1:15]
  ...
  ## ALT is always the minor allele encode so major (REF) is 1 not 0 - like it is now.
    ...
    genotypes[genotypes=="NA" | is.na(genotypes)]<-"3 3"
    genotypes[genotypes=="0/0"]<-"2 2" ## encode 2
    genotypes[genotypes=="0/1"]<-"1 2" ## encode 1
    genotypes[genotypes=="1/1"]<-"1 1" ## encode 0

    alleles<-cbind(c(1:length(the.indel[,"ALT"])),the.indel[,"ALT"],the.indel[,"REF"],0)
    alleles[1:3,]

  x<-1
 
  dim(genotypes)
     data.geno<-apply(as.matrix(c(1:dim(genotypes)[2])),2,function(x,genotypes,alleles){
        print(x)
        x<-as.integer(x)
        geno<- paste(genotypes[,x],collapse=" ")
        geno<-as.integer(unlist(strsplit(geno,split=" ")))
        geno<-matrix(data=geno,nrow=dim(genotypes)[1],ncol=2)
        geno<-cbind(c(1:dim(geno)[1]),geno)
        alleles[geno]
      },genotypes,alleles)
  ...
  fam<-cbind(sample.labels,sample.labels,0,0,-9,-9)
  genotypes<-cbind(,genotypes)
   ...
   write.table(cbind(the.indels[,c("chr")],SNP,0,the.indels[,c("start","ALT","REF")]),file=paste(root, ".map", sep = ""),col.name=FALSE,row.names=FALSE,quote = FALSE) ## ALT is A1
...
## the.indels<-a.indel
## root<-"test"
## the.indels<-indels[position.filter.QC,]
## root<-paste(project.name,"plink",counter,sep="_")
############################## WARNING THIS SUBROURINE IS NOT WORKING in 2015 was in 2014 I don't know why, have tried to fix!
############################## the header bit id correct
############################## the read function however appears to work,,, try to engineer off that 
...
read.plink.raw<-function(root){ ## reda in a bed file add bim information do not mess with A1 or A2 just leave as is
  ...
  genotypes[,"start"]<-gsub("^\\s+","",genotypes[,"start"])
   genotypes[,"chr"]<-gsub("^\\s+","",genotypes[,"chr"])
...
read.plink<-function(root){ ## reda in a bed file add bim information do not mess with A1 or A2 just leave as is
  ...
  ### flip genotypes of t o.flip  to.flip HAve been flipped BUT genotypes have NOT BEEN
...
## indels,
## the.sample.file<-sample.files[isamp]
## ,vcf.type,format.types,info.types,info.class,num.cores

process.GATK.indels.Mercury<-function(indels,the.sample.file,vcf.type,format.types,info.types,info.class,num.cores){
  #   print(dim(indels))
 colnames(indels)[ colnames(indels)=="CHR"]<-"chr"
  ...
  ##   the.col<-info.types[i.info.col]
  ##   posns.in.flatten.info<-grep(paste("^",the.col,"$",sep=""),info.labels,fixed=FALSE) ## can have AC or AC1 etc correct make 01/21/2013
  ##   missing<-is.na(posns.in.flatten.info)
  ##   sum(missing)
  ##   posns<-info.index[posns.in.flatten.info]
  ...
  ### assumes samples are listed after FORMAT COLUMN
  info.sample<-{}
  format.posn<-match("INFO",colnames(indels))
  ...
  ########Multicore run with foreach ## Done columnwise and Format the same for each column
  print(paste("START processing",samples.processing,"samples", " in, ", dim(indels), " , file ",the.sample.file,sep=" "))  
  info.sample<-foreach(info2=iter(indels[,c((format.posn+1):(format.posn+samples.processing))], by='col',chunksize=1), .combine=cbind,.multicombine=TRUE,.inorder=TRUE) %dopar% info.parse.Mercury(info2,format.types,format.string,complex.format)
  ###
  print(paste("Finished processing",samples.processing,"samples",sep=" "))  
  ...
  #################### FINISHED processing individual filters   indels<-indels.ori[c(1000,grep("1/2",indels.ori[,10])[1:3], grep("/3",indels.ori[,10])) ,]
  TYPE<-rep(names(the.sample.file),times=dim(indels)[1])
  filter.posn<-match("INFO",colnames(indels))
  
  indels<-cbind(subset(indels,select=1:filter.posn),TYPE,info.sample) ## use subset here incase indels has only 1 element
...
} # end function  process.GATK.indels.MUrcury
...
#info2<-indels[,78]
info.parse.Mercury<-function(info2,format.types,format.string,complex.format=FALSE){
  
      no.reads<-(grepl("./.",info2,fixed=TRUE) | (info2=="."))  #   (info2=="./.") | (info2==".") # no reads so no information sum(no.reads) changes Nov 4 2013 seen: "./.:.:1"
      info2[no.reads]<-gsub(", ",":",toString(rep("NA",times=length(format.types)))) #  this will be the longest required

      ref<-(grepl("0/0",info2,fixed=TRUE)) 
        info2[ref]<-paste(info2[ref],gsub(", ",":",toString(rep("50",times=length(format.types)-1)) ),sep=":")
       info2<-strsplit(info2,split=":")  # fastest  3.890   0.010   3.894

      test<-unlist(lapply(info2,length))
      test<-test!=length(format.types)
      if(sum(test)>0) {print(paste("Warning", sum(test) ," genotypes are in error set to 0/0"))

                       info2.fix<-paste(rep("0/0",times=sum(test)),gsub(", ",":",toString(rep("50",times=length(format.types)-1)) ),sep=":")
                       info2.fix<-strsplit(info2.fix,split=":") 
                       info2[test]<-info2.fix
                     }
      ...
      len.info2<-length(info2)
      info2<-unlist(info2)
  #    print(paste(num.info2,len.info2))
      dim(info2)<-c(num.info2,len.info2)  
      info2<-t(info2)
      colnames(info2)<-format.types
      info2
...
filtered.genotype.summary<-function(indels,the.samples,prefix="",suffix="",het.read.thresh=20,het.low=0.2,het.high=0.80,het.low.ref=0.05,het.high.alt=0.95,depth.het.low=7,depth.homo.low=5, min.gq = 20){
  ########################### ALL ###############
...
filtered.genotype.old<-function(indels,the.samples,prefix="",suffix="",het.read.thresh=20,het.low=0.2,het.high=0.80,het.low.ref=0.05,het.high.alt=0.95,depth.het.low=7,depth.homo.low=5){
  ########################### ALL ###############
  ## applied filters to genotypes and then makes a summary
...
#x<-allele.depths.group[,1]
#x<-indel
#x<-indel[,colnames(indel)[!grepl(".AD$",colnames(indel))]]
#x<-chh[1:2,4:6]

allele.summary.with.threshold.apply<-function(x,threshold=1){
  missing<-is.na(x) | x=="NA,NA" | x=="NA" | !grepl(",",x) # not an NA must have a comma
  x<-x[!missing]
  num.hetero<-length(x)
 # print(threshold)
 #threshold<-1
  if(num.hetero!=0){    
    ...
    pass.thresh<-x[2,]>=threshold
       if(sum(pass.thresh)==0){ the.counts<-c(0,0,0,0,0)
                           }else{
      the.counts<-apply(as.matrix(x[,pass.thresh]),1,function(xx) sum(xx,na.rm=TRUE))     
      the.counts<-c(the.counts[2],the.counts[1],signif((the.counts[2]/(the.counts[2]+the.counts[1]))*100,digits=3),sum(pass.thresh),length(pass.thresh))
                              }
  }else{
   the.counts<-c(0,0,0,0,0) ## all values missing
  }
  return(the.counts)
}

## threshold<-10
## ## a.indel[c("chr13:21729956:21729956:A:G:snp","chr2:198363406:198363406:C:T:snp"),normals]
## ## a.indel[c("chr13:21729956:21729956:A:G:snp","chr2:198363406:198363406:C:T:snp"),normals.AD]
## indels<-a.indel[c("chr15:40675107:40675107:C:T:snp","chr19:50169131:50169131:C:T:snp","chr17:7578263:7578263:G:A:snp","chr17:7574003:7574003:G:A:snp"),]
## the.samples<-PDs
## the.samples<-gsub(".GT","",normals)

#indels<-final.FAD
#the.samples<-"10694"

alt.reads.reference.calls<-function(indels,the.samples,AD.extension="FAD", threshold=1,prefix="",suffix=""){ ##AN
#alt.reads.reference.calls<-function(indels,the.samples,threshold=1,prefix="",suffix=""){  
  have.AD<-paste(the.samples,AD.extension,sep=".") %in% colnames(indels) ##AN
  #have.AD<-paste(the.samples,"AD",sep=".") %in% colnames(indels)
  if(sum(have.AD)!=0){
    allele.depths.group<-indels[,paste(the.samples,AD.extension,sep=".")]  ##AN
    #allele.depths.group<-indels[,paste(the.samples,"AD",sep=".")]
    ...
    print("No .FAD columns in input data:alt.reads.reference.calls") ##AN
...
alt.reads.Non.reference.calls<-function(indels,the.samples,AD.extension="FAD",threshold=1,prefix="",suffix=""){
...
genotypes<-indels[,paste(the.samples,"GT",sep=".")]
  ...
  allele.depths.group[genotypes=="0/0" | genotypes=="NA" | is.na(genotypes) ] <- NA
...
summary.depths.group.threshold<-t(summary.depths.group.threshold)
  ...
  print("No .FAD columns in input data:alt.reads.reference.calls")
  summary.depths.group<-matrix(data=0,nrow=dim(indels)[1],ncol=3)
}
...
## ##  normal.allele.freq<-normal.alt.counts[test,"Read.Balance"]/100
## ## ## tumor.sample.major.counts<-cancer.alt.counts.true[test,"REF.reads"]
## ## ## tumor.sample.minor.counts<-cancer.alt.counts.true[test,"ALT.reads"]

## ##  normal.allele.freq<-normal.alt.counts[test,"Read.Balance"]/100
## ## tumor.sample.major.counts<-Control.alt.counts[test,"REF.reads"]
## ## tumor.sample.minor.counts<-Control.alt.counts[test,"ALT.reads"]
...
#this uses both tail
.f<- function (){
genotype.p.values<-function(indels,the.samples,AD.extension="FAD",normal.allele.freq,cellularity,alt.count.thresh=1){   ##Achal
#genotype.p.values<-function(indels,the.samples,AD.extension="AD",normal.allele.freq,cellularity,alt.count.thresh=1){
  # use  p[true.sample.minor.counts < alt.count.thresh]<-1 default is less than one
  if(is.null(dim(indels))){indels<-t(as.matrix(indels))}
  cellularity<-as.matrix(cellularity)
  if(dim(cellularity)[2]!=2){print("Error cellularity matrix must be col1=samples, col2=cellulary with 1=all tumor")}
  ...
  if(sum(have.AD)!=0){
    ...
    posns<-match(the.samples,cellularity[,1]) ## assumes first col is sample name
    missing<-is.na(posns)
    the.samples[missing]
    if(sum(missing)>0){
      
      print(paste("Cellualarity of",the.samples[missing],"missing-> using median"))
      a.median<-median(as.numeric( cellularity[posns[!missing],2]),na.rm=TRUE) ## assumes first col is cellularity 
      extra<-cbind(the.samples[missing],a.median)
      colnames(extra)<-colnames(cellularity)
      cellularity<-rbind(extra,cellularity)
      posns<-match(the.samples,cellularity[,1]) ## assumes first col is sample name
      missing<-is.na(posns)
    }
    
    
    cellularity<-cellularity[posns,]
    cellularity[is.na(as.numeric(cellularity[,2])),2]<-median(as.numeric( cellularity[!is.na(as.numeric(cellularity[,2])),2]),na.rm=TRUE)
    
    
    
    
    posns<-match(rownames(indels),names(normal.allele.freq))
    missing<-is.na(posns)
    normal.allele.freq<-normal.allele.freq[posns]
    
    
    if(sum(missing)>0){print("Error estimates of normal.all.freq missing set to 0.001")
                       normal.allele.freq[is.na(normal.allele.freq)]<-0.001
                       names(normal.allele.freq)[is.na(normal.allele.freq)]<-rownames(indels)[is.na(normal.allele.freq)]    
    }
    
    isample<-1
    p.matrix<-matrix(data=1,nrow=dim(indels)[1],ncol=length(the.samples))
    
    for(isample in 1:length(the.samples)){
      #  print(isample)
      p.matrix[,isample]<-poissonian.p.value.using.AD(indels[,paste(the.samples[isample],AD.extension,sep=".")],normal.allele.freq, cellularity[cellularity[,1]==the.samples[isample],2],alt.count.thresh)
      #   p.matrix[,isample]<-poissonian.p.value.using.AD(subset(indels,select=paste(the.samples[isample],"AD",sep=".")),normal.allele.freq, cellularity[cellularity[,1]==the.samples[isample],2])
    }
    colnames(p.matrix)<-the.samples
    rownames(p.matrix)<-rownames(indels)
    
    
  }else{
    print("Error no AD columns passing ALL")
    p.matrix<-matrix(data=1e-99,nrow=dim(indels)[1],ncol=length(the.samples))
  }
  
  ## found.genotype<-  p.matrix<=p.threshold
  
  ## p.matrix[found.genotype]<-"0/1" # p.threshold<- 0.0026
  ## p.matrix[!found.genotype]<-"0/0"
  p.matrix
}
}
...
poissonian.model.position<-function(normal.allele.freq,tumor.sample.major.counts, tumor.sample.minor.counts,av.normal.in.tumor=1){
  # av.normal.in.tumor =1 for no contaminaltion
  
  ## tumor.sample.major.counts<-as.numeric(indels[,tumor.sample.major.counts.COL])
  ## tumor.sample.minor.counts<-as.numeric(indels[,tumor.sample.minor.counts.COL])
  
  
  normal.coverge.in.tumor<-(tumor.sample.major.counts+ tumor.sample.minor.counts)*(1-as.numeric(av.normal.in.tumor))
  ...
  true.sample.cov<-(tumor.sample.major.counts+ tumor.sample.minor.counts)- normal.coverge.in.tumor
  true.sample.major.counts<-tumor.sample.major.counts - normal.major.in.tumor
  true.sample.minor.counts<-tumor.sample.minor.counts - normal.minor.in.tumor
  ...
  normal.allele.freq[normal.allele.freq<0.001]<-0.001 ### set a minimum low allele freqency so maths does not go crazy
  ...
  stats<-cbind(z,p)
  colnames(stats)<-c("Z-score","P-value")                                   
  ## true.sample.cov*normal.allele.freq
  ##  tumor.sample.minor.counts
  ## stats
...
## tumor.sample.major.counts<-normal.alt.counts[test,"REF.reads"]
## tumor.sample.minor.counts<-normal.alt.counts[test,"ALT.reads"]
## corrected.geno<-genotype.correction(a.indel[test,],Control.alt.counts[,"Read.Balance"]/100,cellularity)


## enotype.p.values( ,,Control.alt.counts[pass,"Read.Balance"]/100,cellularity)
## genotype.p.values(a.indel[snp.only & has.one.geno ,] ,c(cancer,PDs),Control.alt.counts[snp.only & has.one.geno,"Read.Balance"]/100,cellularity)

## a.indel[pass,],cancer,Control.alt.counts[pass,"Read.Balance"]/100,cellularity
## indels<-a.indel[snp.only & has.one.geno ,]
## the.samples<-cancer
## normal.allele.freq<-Control.alt.counts[snp.only & has.one.geno,"Read.Balance"]/100
...
## indels<-a.indel[test,]
## the.samples<-c(cancer)
## normal.allele.freq<-Control.alt.counts[test,"Read.Balance"]/100

genotype.p.values<-function(indels,the.samples,AD.extension="FAD",normal.allele.freq,cellularity,alt.count.thresh=1,AD.lower.tail = FALSE){  ##Achal
#genotype.p.values<-function(indels,the.samples,AD.extension="AD",normal.allele.freq,cellularity,alt.count.thresh=1,AD.lower.tail = FALSE){
# use  p[true.sample.minor.counts < alt.count.thresh]<-1 default is less than one
 #  AD.lower.tail use falue if trying to detect more alt reads
   print(alt.count.thresh)
   print(AD.lower.tail)
  ...
  for(isample in 1:length(the.samples)){
  #  print(isample)
   p.matrix[,isample]<-poissonian.p.value.using.AD(indels[,paste(the.samples[isample],AD.extension,sep=".")],normal.allele.freq, cellularity[cellularity[,1]==the.samples[isample],2],alt.count.thresh,AD.lower.tail)
  #   p.matrix[,isample]<-poissonian.p.value.using.AD(subset(indels,select=paste(the.samples[isample],"AD",sep=".")),normal.allele.freq, cellularity[cellularity[,1]==the.samples[isample],2])
 }
  colnames(p.matrix)<-the.samples
  rownames(p.matrix)<-rownames(indels)
  ...
  print("Error no FAD columns passing ALL")  ##Achal
  #print("Error no AD columns passing ALL")
  p.matrix<-matrix(data=1e-99,nrow=dim(indels)[1],ncol=length(the.samples))
}
...
poissonian.p.value.using.AD<-function(genotypes,normal.allele.freq,av.normal.in.tumor=1,alt.count.thresh=1,AD.lower.tail = FALSE){
   ## print(alt.count.thresh)
   ## print(av.normal.in.tumor)
    missing<-is.na(genotypes) | genotypes=="NA,NA" | genotypes=="NA" | !grepl(",",genotypes)
 #   sum(missing)
    genotypes[missing]<-"0,0"  #### AD not formated correctly
    x<-strsplit(genotypes,split=",")
    problems<-unlist(lapply(x,length))!=2
 #   sum(problems)
    if(sum(problems)>0){print("ERROR can AD not of length 2 set to 0,0")
                      genotypes[problems]<-"0,0"
                       x<-strsplit(genotypes,split=",")
                      }

    x<-as.numeric(unlist(x))
  dim(x)<-c(2,length(genotypes))
  tumor.sample.major.counts<-x[1,]
  tumor.sample.minor.counts<-x[2,]

  
normal.coverge.in.tumor<-(tumor.sample.major.counts+ tumor.sample.minor.counts)*(1-as.numeric(av.normal.in.tumor)) # line A

                           
 normal.minor.in.tumor<-normal.coverge.in.tumor*normal.allele.freq # line AA # number of alt allele in tumor
 normal.major.in.tumor<- normal.coverge.in.tumor-normal.minor.in.tumor ## # line BBcount of major or genomic reference allele (forward strand genomic reference)
# normal.major.in.tumor+ normal.minor.in.tumor
 ...
 true.sample.major.counts<-tumor.sample.major.counts - normal.major.in.tumor #
 true.sample.minor.counts<-tumor.sample.minor.counts - normal.minor.in.tumor
# true.sample.cov<-true.sample.major.counts+ true.sample.minor.counts

 

 ## true.sample.minor.counts<-as.integer(true.sample.minor.counts)
 ## true.sample.major.counts<-as.integer( true.sample.major.counts)
 ## true.sample.cov<-(tumor.sample.major.counts+ tumor.sample.minor.counts)
 ...
 ## summary[,paste(sample.names[3],"Allele Freq",sep=".")]
 #sort(normal.allele.freq)
# normal.allele.freq[normal.allele.freq<0.005]<-0.005 ### set a minimum low allele freqency so maths does not go crazy
 normal.allele.freq[normal.allele.freq<0.001]<-0.001 ### set a minimum low allele freqency so maths does not go crazy
 true.sample.cov[true.sample.cov<=0]<-1  ## case when normal has coverge but tumor does not!
...
##  true.sample.minor.counts<-   8
 ...
 z<- (true.sample.minor.counts- true.sample.cov*normal.allele.freq) / (sqrt(true.sample.cov*normal.allele.freq*(1-normal.allele.freq)))
    ...
    if(AD.lower.tail=="BOTH"){
      p<-2*(pnorm(abs(z), mean = 0, sd = 1, lower.tail = FALSE, log.p = FALSE))
    }else{
      p<-2*(pnorm(z, mean = 0, sd = 1, lower.tail = AD.lower.tail, log.p = FALSE))
      }##this is the 2 tailed

  p[true.sample.minor.counts <= alt.count.thresh]<-1 # if  true.sample.minor.counts < alt.count.thresh set to not significant

 ## stats<-cbind(z,p)
 ## colnames(stats)<-c("Z-score","P-value") 
  
## true.sample.cov*normal.allele.freq
##    true.sample.minor.counts
##  tumor.sample.minor.counts
## stats
...
## other.genes<-sapply(
##                     x<-unlist(strsplit(table1[,table1.match.col][poly.genes],split=table.delim)[4])
##                     #,delimit.collapse,tab=tab2,tab.match.col=tab2.match.col)
## tab<-tab2
## tab.match.col<-tab2.match.col

delimit.collapse<-function(x,tab,tab.match.col){
  posns<-match(x,tab[,tab.match.col])
  missing<-is.na(posns)
  data<-as.matrix(tab[posns[!missing],])
  if(dim(data)[2]!=length(colnames(tab))){data<-t(data)}
...
## match.cols.and.collapse(the.loci.list[chk],1,conversion,"gene.id",,"delimit")

##  table1<-the.loci
##  table1.match.col<-"gene.id"
## table.delim=";"
##  tab2<-conversion
##  tab2.match.col<-"Entrez Gene ID (mapped data supplied by NCBI)"
##  tab2.collapse.cols<-"Approved Symbol" #colnames(conversion)
## tab2.collapse.cols<-colnames(conversion)
##  gene.collapse<-"delimit"




match.cols.and.collapse.table<-function(table1,table1.match.col,table.delim,tab2,tab2.match.col,tab2.collapse.cols,gene.collapse){ # match genes names (might be more than one) in list  to a table and collapse

  ## the.loci.list<-lapply(strsplit(table1[,table1.match.col],split=table.delim),length)
  ## chk<-lapply(the.loci.list,length)>1

  
  num.genes<-unlist(lapply(strsplit(table1[,table1.match.col],split=table.delim),length))                                     # gived back table of dimension of indel table gene.collapse="delimit","boolean","sum"
  ...
  a.desc.table<-matrix(data=NA,nrow=length(table1[,table1.match.col]),ncol=length(tab2.collapse.cols))
  colnames(a.desc.table)<-tab2.collapse.cols
  a.desc.table<-as.data.frame(a.desc.table,stringsAsFactors=FALSE)
  
  posns<-match(table1[!poly.genes,table1.match.col],tab2[,tab2.match.col]) ## match the single items first to get the out-of -the -way
  missing<-is.na(posns)
  #sum(missing)  sum(!missing)
  ## sum(!poly.genes)
  ## length(posns[!missing])
  
  ## length(a.desc.table[!poly.genes,tab2.collapse.cols[1]][!missing])
  ## length(as.character(tab2[posns[!missing],tab2.collapse.cols[1]]))
  ...
  if(!sum(poly.genes)==0){
    #print(gene.collapse) table1[,table1.match.col][poly.genes]
    if(gene.collapse=="boolean"){
      ...
      #     system.time(
      other.genes<-sapply(strsplit(table1[,table1.match.col][poly.genes],split=table.delim),delimit.collapse,tab=tab2,tab.match.col=tab2.match.col)
...
## seqname<-names(human.chromlens)[1]

## bamFile<-the.bam.files[ii]
## human.chromlens<-human.chromlens
...
extract.value.from.info.coverage<-function(info,match.string){ #will be fooled if have GGMAF=  #ok if GMAF not present need [^;]
match.string.general<-paste(match.string,"[a-zA-Z0-9,\\.]*",sep="") #regexec(";GMAF=[a-zA-Z0-9,\\.]*;",indels[has.gmaf,"INFO"]) ";" not needed
match<- regexec(match.string.general,info)
start<-unlist(match)
length<-unlist(lapply(match,function(x)  attr(x,"match.length")))
...
extract.value.from.format.coverage<-function(info,match.string){ #will be fooled if have GGMAF=  #ok if GMAF not present need [^;]
match.string.general<-paste(match.string,"[a-zA-Z0-9\\ \\.]*",sep="") #regexec(";GMAF=[a-zA-Z0-9,\\.]*;",indels[has.gmaf,"INFO"]) ";" not needed
match<- regexec(match.string.general,info)
start<-unlist(match)
length<-unlist(lapply(match,function(x)  attr(x,"match.length")))
...
extract.value.from.DS.coverage<-function(info,match.string){ #will be fooled if have GGMAF=  #ok if GMAF not present need [^;]
match.string.general<-paste(match.string,"[a-zA-Z0-9,:\\+\\ \\.\\-]*",sep="")  #regexec(";GMAF=[a-zA-Z0-9,\\.]*;",indels[has.gmaf,"INFO"]) ";" not needed
match<- regexec(match.string.general,info)
start<-unlist(match)
length<-unlist(lapply(match,function(x)  attr(x,"match.length")))
...
#	print(a.readgroup["DS",i.ar])
  if(!grepl("^NA",a.readgroup["DS",i.ar]) & !grepl("TruD:Agl1.2",a.readgroup["DS",i.ar]) & !grepl("TruD:TruX",a.readgroup["DS",i.ar]) & !grepl("NEBuD;TruX",a.readgroup["DS",i.ar])  & !grepl("KapD:NimX3",a.readgroup["DS",i.ar])   & !grepl("TruD:NimX3",a.readgroup["DS",i.ar]) & !grepl("TruD:NimX",a.readgroup["DS",i.ar]) & !grepl("NxtD:NxtX",a.readgroup["DS",i.ar]) &  !grepl("NxtD:NxtXR",a.readgroup["DS",i.ar])  & !grepl("NxtD:NxtXE",a.readgroup["DS",i.ar])  & !grepl("TruD:NimX",a.readgroup["DS",i.ar]) & !grepl("TruD;",a.readgroup["DS",i.ar]) ){print("WARNING unknown capture technology in description field - see subroutine get.readgroup.info");print(a.readgroup["DS",i.ar])}
...
if(the.capture[from.bam]=="NEBuD"){targets.file<-"Human_Exome_Targets_illumina_v2_hg19_targets.RData"} #(version=="v2")TruD;Agl1.2
    ...
    if(the.capture[from.bam]==""){targets.file<-"Aglient_clinical_TargetedRegions_hg19_targets.RData"}
}
